...
namespace Core.Utilities.Helper
{
    public class FileHelper
    {
        ...
        public static IResult Upload(IFormFile file)
        {
            ...
            var validType = CheckFileValidType(type);
            var randomName = Guid.NewGuid().ToString();
            if (validType.Message != null)
            {
                return new ErrorResult(validType.Message);
            }
        ...
        private static IResult CheckFileExists(IFormFile file)
        {
            ...
            return new ErrorResult(CoreMessages.FileDoesntExist);
        }
        private static IResult CheckFileValidType(string type)
        {
            if (type != ".jpeg" && type != ".png" && type != ".jpg")
            {
                return new ErrorResult(CoreMessages.WrongFileType);
            }
            return new SuccessResult();
        }
